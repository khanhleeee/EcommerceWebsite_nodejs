...
const showProductList = async(req, res, next) => {
    ...
    let p = await Product.find({'skus.sku.sizes.qty': {$ne: 'VC0AAH'}});
...
const showProductDetail = async(req, res, next) => {
    ...
    res.render('TabProduct/productdetail', { layout: 'mainClient.hbs', userInfo: mongooseToObject(req.user), sku, size, product: mongooseToObject(product), color: multipleToObject(colors), category: multipleToObject(categories), user: mongooseToObject(req.user) })
}
