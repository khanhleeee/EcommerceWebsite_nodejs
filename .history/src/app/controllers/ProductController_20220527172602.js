...
const showProductDetail = async(req, res, next) => {
    ...
    res.render('TabProduct/productdetail', { layout: 'mainClient.hbs', userInfo: mongooseToObject(req.user), sku, size, isOutOfStock, product: mongooseToObject(product), color: multipleToObject(colors), category: multipleToObject(categories), user: mongooseToObject(req.user), showUserCmt: mongooseToObject(showUserCmt) })
}
