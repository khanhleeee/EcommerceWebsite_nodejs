...
const showAdmin = async(req, res, next) => {
    ...
    const dataHour = await Order.aggregate(
            ...
            {$match: { "orderStatus": "done" }},
        {
            $group: 
            {
                ...
                price: 
                { $sum: {
                    // $convert: {input: '$orderTotal', to : 'int', onError: '',onNull: ''}
                    $convert: {
                        input: {
                            $reduce: {
                                input: {
    ...
    // price: { $sum: { $convert: { input: "$orderTotal", to: "int", onError: "NaN", onNull: 2}}},
    const objHour = Object.assign({}, dataHour);
    for(var i in dataHour) {
        console.log(dataHour[i].price);
        var x = Intl.NumberFormat().format(dataHour[i].price);
    }
