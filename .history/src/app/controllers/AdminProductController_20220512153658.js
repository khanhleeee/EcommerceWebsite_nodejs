...
const showEditSku = async(req, res, next) => { 
    ...
    for(var i in product.skus) {
        if(product.skus[i] === req.body.sku)
            sku = product.skus[i];
    }
    res.render('TabAdmin/admin-edit-form', { layout: 'mainAdmin.hbs', product: mongooseToObject(product), sku, user: mongooseToObject(user) });
}
