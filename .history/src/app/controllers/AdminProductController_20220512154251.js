...
const showEditSku = async(req, res, next) => { 
    ...
    for(var i in product.skus) {
        if(product.skus[i].sku === req.params.sku)
            sku = product.skus[i];
    }
    // console.log(sku)
    res.render('TabAdmin/admin-skus-edit', { layout: 'mainAdmin.hbs', product: mongooseToObject(product), sku: sku, user: mongooseToObject(user) });
}
...
module.exports = { showProductList, showCreateList, createProduct, createSkus, showEditProduct, showEditSku, updateProduct, deleteProduct }