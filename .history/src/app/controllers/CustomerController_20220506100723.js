...
const showCustomerTransaction = async(req, res, next) => {
    const userInfo = await User.findById(req.user._id);
    const order = await Order.find({ name: userInfo.name });
    ...
    res.render('TabCustomer/cus-transaction', { layout: 'mainClient.hbs', orderCus: multipleToObject(order), products: products, user: mongooseToObject(req.user), userInfo: mongooseToObject(userInfo) });
}
